...
/**
 * The sample chain is 5 + 9 + 11 days, so the critical path is 25 — short
 * enough to check by hand, which is the point of a fixture for an algorithm.
 */
class CriticalPathIT extends AbstractIT {

    @Test
    void computesTheChainAsTheCriticalPath() throws Exception {
        Long project = firstProjectOf("uma");

        mockMvc.perform(get("/api/v1/projects/{p}/schedule/critical-path", project)
                .with(as("uma")))
    ...
    void givesTheStandaloneTaskFloatAndTheChainNone() throws Exception {
        Long project = firstProjectOf("vera");
